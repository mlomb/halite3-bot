...
static std::string get_string(std::unordered_map<std::string, std::string>& map, const std::string& key) {
    ...
    if (it == map.end()) {
        out::Log("Error: constants: server did not send " + key + " constant.");
        exit(1);
    }
    return it->second;
}
...
static bool get_bool(std::unordered_map<std::string, std::string>& map, const std::string& key) {
    ...
    out::Log("Error: constants: " + key + " constant has value of '" + string_value +
...
void hlt::constants::populate_constants(const std::string& string_from_engine) {
    ...
    if ((tokens.size() % 2) != 0) {
		out::Log("Error: constants: expected even total number of key and value tokens from server.");
        exit(1);
    }
